...
export default async function ArticlePage(props) {
  ...
  if (!article) {
    return <div className="p-10 text-center text-xl font-bold text-red-500">Статья не найдена</div>;
  }

  const title = article.titleRu || article.title || 'Без названия';
  const summary = article.summaryRu || article.description || article.summary || '';

...
const getSubCategoryKey = (product, category) => {
  if (category === "cheese") {
    if ([1, 2, 3, 14, 21, 18, 19].includes(product.id)) return "gratedCheese";
    if ([7, 12, 9, 17].includes(product.id)) return "blockCheese";
    return "otherCheese";
  }
  if (category === "cream") return "creams";
  if (category === "sauce") return "sauces"; // اضافه شدن تشخیص سس
  if (category === "butter") return "butters"; // اضافه شدن تشخیص کره
  return "allProducts";
};

// تعیین ترتیب دلخواه برای نمایش گروه‌ها (سایر پنیرها قبل از پنیرهای قالبی قرار گرفت)
...
export default function ProductGrid({
  ...
  return (
    <div className="w-full py-5 max-w-7xl mx-auto px-4 xs:px-12 md:px-8 mt-6 xs:mt-10 mb-10 xs:mb-20 animate-fade-in-up">
      {/* فیلتر کردن و نمایش بر اساس ترتیبی که در CATEGORY_ORDER مشخص کردیم */}
      {CATEGORY_ORDER.filter((key) => groupedProducts[key]).map(
        (subCategoryKey) => {
